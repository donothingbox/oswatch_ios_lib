...
static NSMutableArray *detailsPacketArray;
static NSInteger packetCount;
static NSInteger detailsPacketCount;
...
-(void)processIncomingData:(unsigned char *) data length:(int) length
{
    ...
    switch (data[BYTE_EVENT_APP_ACTION]) {
        ...
        case RSS_APP_ACTION_LOAD_PACKET:
            ...
            [self sendRequestedPacket:[self byteToInteger:&data[3]]];
            break;
        case RSS_APP_ACTION_LOAD_DETAIL_METADATA:
            NSLog(@"RSS LOAD DETAIL METADATA DETECTED: %d", data[3]);
            [self sendRSSDetailMetadata];
            break;
        case RSS_APP_ACTION_LOAD_DETAIL_TITLE_BLOCK:
            NSLog(@"RSS LOAD DETAIL BLOCK TITLE DETECTED: %d", data[3]);
            feeds = [rssParserObject getFeeds];
            [self sendRSSDetailLine:(NSString *)[feeds[[self byteToInteger:&data[3]]] objectForKey:@"title"]];
            break;
        case RSS_APP_ACTION_LOAD_DETAIL_DESCRIPTION_BLOCK:
            NSLog(@"RSS LOAD DETAIL BLOCK DESCRIPTION DETECTED: %d", data[3]);
            feeds = [rssParserObject getFeeds];
            [self sendRSSDetailLine:(NSString *)[feeds[[self byteToInteger:&data[3]]] objectForKey:@"description"]];
            break;
        case RSS_APP_ACTION_LOAD_DETAIL_PACKET:
            NSLog(@"RSS LOAD DETAIL PACKET DETECTED: %d", data[3]);
            [self sendRequestedDetailPacket:[self byteToInteger:&data[3]]];
            break;
        default:
...
-(IBAction)sendRSSDetailMetadata{
    UInt8 buf[6] = {0x03, 0x04, 0x09, 0x01, 0x01, 0x01};
    [[self getBLEDelegate] sendData:buf];
}

-(IBAction)sendRSSDetailLine:(NSString*)mySentence{
    
    
    NSLog(@"String to Send: %@", mySentence);
    UInt8 buf[3] = {0x03, 0x05, 0x04};
    ...
    //Reset packet array
    detailsPacketArray = [NSMutableArray array];
    detailsPacketCount = 0;
    ...
    for(int i = 0;i<=numberOfPackets;i++) {
        ...
        [detailsPacketArray addObject:packet];
        detailsPacketCount++;
    }
    
    buf[2] = *[[self getBLEDelegate] integerToByte:detailsPacketCount];
    //NSLog(@"Sending First String: %@", packetArray[0]);
    
    NSMutableData *dataObj = [[NSMutableData alloc] initWithBytes:buf length:3];
    [dataObj appendData:[detailsPacketArray[0] dataUsingEncoding:NSUTF8StringEncoding]];
...
-(IBAction)sendRequestedDetailPacket:(NSInteger)packetID{
    //NSLog(@"Requesting next packet: %ld", (long)packetID);
    UInt8 buf[3] = {0x03, 0x06, 0x04};
    NSMutableData *dataObj = [[NSMutableData alloc] initWithBytes:buf length:3];
    
    [dataObj appendData:[detailsPacketArray[packetID] dataUsingEncoding:NSUTF8StringEncoding]];
